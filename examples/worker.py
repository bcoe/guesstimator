"""
This exmaple demonstrates how sampy might be used by a worker in a distributed system.
...
while True:
    ...
    operations = redis.incr('sampy_example_actual_operation_count')
    
    # reader.py must have reset the operation count.
    if last_operations_count > operations:
        sampy.writes_performed = 0
    ...
    if (operations % 10000) == 0:
        ...
        estimated_worker_count = int( (float(operations) / (float(sampy.writes_performed) / 0.05)) + 0.5 )
